...
In the near future the number of humans developing software professionally will drop a hundredfold. Maybe more.
...
Ok, ok. It is *actually* just my opinion, but as open-minded as I am, I fail to find evidence to the contrary.

And, yes, I know saying this puts me square in a group along with a bunch of people who wear tinfoil hats and fear glowies, and a whole lot of people who have little idea of what software development really is. However I have almost 25 years of experience as a software engineer. I think I have some convincing arguments to back my claim up. So, please, don't be blinded by the shininess of my headgear and hear me out.

First and foremost, I'm not saying software developers will get replaced by AI doing their jobs (THEY TERK ER JERBS!). I do *not* think the future is essentially the same software shops creating more or less the same software, but instead of humans, it's AI agents doing the work.
...
So my premise is this: AI makes for a decent software engineer and it's only gonna get cheaper.
...
The end user wants us to ship and ship fast, but of course in order to do so, we, as engineers, as humans, have to lay the groundwork to make it possible. We invest in these meta-solutions to the point where entire sub-industries exist whose sole job is developing just them. Then software architects combine meta-solutions into a pipeline for creating end user software.

But it is ultimately the end user that's important, right? Software *has* to do something important outside of the software domain. That's where the money comes from. That's why we can afford investments of billions, trillions even, into meta-solutions.
...
But that's not the end game. What I've just described is precisely the thing that I "first and foremost" wasn't going to propose.
...
I am with the critics so far. End users empowered by AI agents can't build the same softare we do.
...
What we don't find in every software is independent programs. It is a good way to build an operating system where there is no task-specific context. There is no concrete job to be done. It has to support users who watch TikTok videos all day *and* users who run simulations that push the frontier of astrophysics. And TikTok, and Astrophysics Frontier Pusher&trade;, and Photoshop do have a task-specific context. They support much more specific workflows (and funflows?) and must provide a user interface with which the user can "tell" the computer what to do. These types of software have a protocol by which user intent is sent from the user interface to the backend and a protocol by which the result is sent back. They need design to allow the user express their intent and accomplish tasks.

Translating the user's intent into the protocol for subsystem communication and devising the data structures to carry that intent and the ones to carry the result back is the job of the software designers and engineers. It's an inherently lossy process.

`$ photoshop foo.png | select-object --coord 123;125 --threshold 0.3 | fill-pixels | gradient --linear --color=green --direction=1,0`

It is technically possible to make Photoshop work like that. It would have an incredibly steep learning curve, yes, but this is almost ImageMagick, and people do use ImageMagick. That's not the most important thing here. It's the protocol I mentioned above. All of these programs need a protocol to exchange the information. It's just that the protocol Photoshop has chosen is shared memory within a single process. Internally the dataflow is likely very similar. It would be impractical to design Photoshop by employing discrete programs for each task. The separation of concerns exists, but it's internal, in a big software monolith.

While it's doubtful that an AI agentic process will be successful in replicating Photoshop, AI agents are well capable of developing each of these programs.

When designing software systems, we focus on allowing users to express their intent. Intent is hard to process. Complex systems allow users to configure or otherwise augment the ways they express their intent until they all [contain an ad hoc, informally-specified, bug-ridden, slow implementation of half of Common Lisp](https://en.wikipedia.org/wiki/Greenspun%27s_tenth_rule). And that's precisely where LLMs shine. LLMs are the ultimate pipe. Yes, Photoshop could have a tool to select an object at the cursor's position and fill it with a green gradient, but we all laugh at how specific this is. The user wants to see how their house would look with a green door. And we *all* know that it's not Photoshop, but a certain nano-scale elongated yellow fruit that people will use to do this nowadays.

So, what LLMs bring to the table is an unparalleled way of translating user intent into actions. Why learn the Lasso tool, when you can just type (or say) "select all puppies"? Why even select? Selection is just a means of subdividing intent into manageable subtasks that can fit the protocol. Convert all puppies to dragons! Done.
...
I am, however, not applying this to humans. I'm applying it to software. I argue that the world is headed towards *the situation* of this situational cost asymmetry.

Once upon a time, in the late 1990s, when I was beginning my first ever experiments with graphics and game development, I was really surprised to learn that it is efficient to render the entire scene into an image and then blit this entire image onto the screen replacing the previously rendered one. I found it really hard to internalize that this is usually significantly *more* efficient than trying to keep track of the parts of the screen that are being affected by the current frame, and only replacing those. My first ever snake game was laboriously clearing the tail section of the snakes and replacing the head section, but once I "got" double buffering, I never looked back.

We're about to enter the world of software double buffering.

If we're Unix-ifying software, we still need those independent programs that the AI will use. Yes, AI can write them, but AI can't possibly do a good enough job designing them.
...
AI agents can create a plan, execute steps, write a piece of code for each step as needed and then throw away everything. The dataflow design is temporary. Specific to a given task. And even if storage and data integrity is mostly safe from the clutches of the robots for now, I wouldn't be surprised if this changes.

When designing a, say CRM system, you will likely devote some time designing the database. You will design queries. You may decide whether to have an ORM or which one. But I put forward that if you give a bare database to an agent and prompt "this is a CRM database, create a graph of the conversion of marketing budget to sales in the previous year" it will likely do a decent job. "This was very slow, make such requests faster in the future.". This will likely be within the capabilities of most agents.

## The Future

I am not saying that people who create software for a living will be gone, I think the prospects of this future will dramatically reduce their number.

Without foundational model improvements a lot of software development will definitely remain in human territory. Arguably the "more important" things: theoretical developments; identifying, fixing, and naturally, exploiting security vulnerabilities; portions trading software; portions of medical software; performance critical optimizations of dataflow in databases, networks, video; perhaps many more concrete tasks. That [person in Nebraska](https://xkcd.com/2347/) thanklessly supporting all modern digital infrastructure since 2003 is definitely safe for the foreseeable future.

I think there will be some growth in data architecture. New jobs like State Architects, or Data Librarians, or Schema Wizards, will emerge, but their output, even if made private and hidden, is highly scalable: it's principles rather than implementations.

The thing is all of those are niche. That is a lot of niches, but the vast majority of software development, where the lossy compression of intent makes users suffer the most, will be gone. That's the software used by most people on a daily basis.

In my head I'm writing off much of the developer tooling we now have, most utility libraries[^4], most web and mobile apps, and practically everything you would find in a stereotypicall office computer: rich text processors, image manipulation software, spreadsheets, diagram software, presentation programs.
